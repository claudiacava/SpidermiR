...
`SpidermiR` allow the user to query, prepare, download network data (e.g. from GeneMania), and to integrate this information with miRNA data 
...
This function, starting from communites obtained from `SpidermiRanalyze_Community_detection`, is able to indicate in which communites are presented a set of biomarker of interest.
...
## Case Study n.1: Role of miRNAs in shared protein domains in Prostate Cancer
...
org<-SpidermiRquery_species(species)
net_shar_prot<-SpidermiRquery_spec_networks(organismID = org[12,],network = "SHpd")
out_net<-SpidermiRdownload_net(net_shar_prot)
geneSymb_net<-SpidermiRprepare_NET(organismID = org[12,],data = out_net)
miRNA_NET<-SpidermiRanalyze_mirna_network(data=geneSymb_net,disease="prostate cancer")
biomark_of_interest<-c("hsa-miR-214","PTEN","FOXO1","hsa-miR-27a")
GIdirect_net_neigh<-SpidermiRanalyze_subnetwork_neigh(data=miRNA_NET,BI=biomark_of_interest)
SpidermiRvisualize_mirnanet(data=GIdirect_net_neigh)
