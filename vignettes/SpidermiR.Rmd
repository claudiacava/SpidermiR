...
- id: ref6
  title: miRandola Extracellular Circulating microRNAs Database.
  author: 
  - family: Colaprico A, Silva TC, Olsen C, Garofano L, Cava C, Garolini D, Sabedot TS, Malta TM, Pagnotta SM, Castiglioni I, Ceccarelli M, Bontempi G, Noushmehr H.
    given:
  journal: Nucleic Acids Res.
  volume: 44
  number: 8
...
The user can download Pharmaco-miR Verified Sets from miRandola database
...
The user can download Pharmaco-miR Verified Sets from PharmacomiR database [@ref3].
...
The user can focus on miRNA have already linked to a particular disease or take all miRNAs. miRNA-gene interactions include data from validated studies (currently, mirTAR, miR2disease, and miRNAwalk [@ref2] [@ref4]).
The miRNA network can be filtered by disease using the name of the disease, as obtained from `SpidermiRquery_disease`.
...
Extracellular/circulating miRNAs include data from mirandola database [@ref5].
...
`SpidermiRanalyze_DEnetworkTCGA` is able to identify using `TCGAbiolinks package` [@ref6] differentially expressed (DE) genes or miRNAs in a TCGA data ( e.g. BRCA:breast camcer) between two classes (e.g. normal and tumour). For more detail, please see [@ref6]. Starting from DE genes or miRNAs the user can select how these biomarkers act in a specific network looking for  the edges between them (using the function `SpidermiRanalyze_direct_subnetwork`). 

```{r, eval = FALSE}
de_int<-SpidermiRanalyze_DEnetworkTCGA(data=miRNA_NET,cancer="BRCA",PlatformCancer="IlluminaHiSeq_RNASeqV2",tumour=c("TCGA-BH-A1FC-11A-32R-A13Q-07","TCGA-AO-A03P-01A-11R-A00Z-07"),
...
This function try to find dense subgraphs in directed or undirected graphs, by optimizing some criteria, using igraph library [@ref7]. 
...
The user can visualize the degree distribution of the network:
...
The user can visualize the adjacency matrix of the network:
...
The user can visualize 3D barplot with a summary representation of the network (number of edges, nodes, miRNAs):
